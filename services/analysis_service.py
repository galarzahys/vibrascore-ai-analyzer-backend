...
- FATURAMENTO_MENSAL: OBRIGATORIO - Preencha SEMPRE o array "faturamento_mensal" com TODOS os meses encontrados nos documentos de faturamento, em ordem cronologica. NUNCA deixe este array vazio se houver qualquer dado de faturamento nos documentos. Cada entrada deve ter: competencia (MM/AAAA), valor (numerico, obrigatorio), variacao_ah (percentual ou null). Se o documento tiver extratos bancarios, DRE ou qualquer dado de receita mensal, extraia e preencha este array. Em caso de duvida sobre o valor exato de um mes, use o valor disponivel mais proximo ou a media do periodo.
...
- Para qsa: inclua dados de PEP, sancoes e perfil do bureau se disponivel. Quando a Declaracao de Imposto de Renda Pessoa Fisica (IRPF/DIRPF) de um socio estiver entre os documentos enviados, identifique o socio pelo CPF constante no cabecalho da declaracao ("CPF:") e cruze com o CPF daquele socio no bureau/QSA. Encontrando a correspondencia:
  - Marque "irpf_disponivel": true. Sem correspondencia, mantenha "irpf_disponivel": false e deixe "bens_arrestaveis" null — NUNCA estime esses valores sem a declaracao.
  - Calcule "bens_arrestaveis" como o valor da linha "TOTAL" da secao "DECLARACAO DE BENS E DIREITOS" (coluna "Situacao em 31/12" do ano-calendario MAIS RECENTE) menos o valor da linha "TOTAL" da secao "DIVIDAS E ONUS REAIS" (mesma coluna; se a secao nao existir ou constar "Sem Informacoes", considere 0 de dividas). Nunca deixe o resultado negativo — nesse caso registre 0.
  - Em "analise_geracional", ACRESCENTE ao final do texto (apos as informacoes de geracao/PEP/sancoes ja previstas) duas frases:
    1) A listagem resumida dos principais bens declarados na secao "DECLARACAO DE BENS E DIREITOS" (ano-calendario mais recente), no formato "Bens declarados: [descricao curta do bem] (R$ valor), [descricao curta do bem] (R$ valor), ...". Use uma descricao curta e objetiva de cada bem (ex: "imovel em Joinville/SC", "veiculo BMW M2", "quotas societarias — Garcia Participacoes"), na ordem em que aparecem na declaracao. Se houver mais de 6 bens, liste os 6 de maior valor e finalize com "e outros N itens de menor valor".
    2) O resumo financeiro no formato: "IRPF: patrimonio declarado de R$ X, dividas de R$ Y (patrimonio liquido pessoal de R$ Z), com [aumento/reducao/estabilidade] patrimonial de [valor ou percentual] entre os dois anos-calendario declarados." Use X = total de bens e direitos, Y = total de dividas e onus reais, Z = bens_arrestaveis, todos do ano-calendario mais recente, e compare com o ano-calendario anterior da mesma declaracao para descrever a variacao.
    Nao crie campos novos nem paragrafos separados — todo esse texto entra na mesma "analise_geracional" ja existente.
